...
async def enrich(request : Request, body: dict = { "ip": get('https://api.myip.com').json()["ip"] }):
    try:
        logger.debug(f"[Server](post_enrich): Accessed from [{ request.client.host }:{ request.client.port }]\nBody: '{ body }'")
        return process(body, api_config)
    except BaseException as e:
        logger.error(f"[Server](post_enrich): { e }")
...
def read_api_configuration(request : Request):
    logger.debug(f"[Server](get_api-configuration): Accessed from [{ request.client.host }:{ request.client.port }]")
    return api_config
...
async def update_api_configuration(request : Request, configuration_file: UploadFile):
    try:
        logger.debug(f"[Server](post_api-configuration): Accessed from [{ request.client.host }:{ request.client.port }]")
        ...
        logger.error(f'[Server](post_api-configuration): { e }')
        return JSONResponse({ "message": "Unsuccess!" }, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def read_log_configuration(request : Request):
    logger.debug(f"[Server](get_log-configuration): Accessed from [{ request.client.host }:{ request.client.port }]")
    return log_config
...
async def update_log_configuration(request : Request, configuration_file: UploadFile):
    try:
        logger.debug(f"[Server](post_log-configuration): Accessed from [{ request.client.host }:{ request.client.port }]")
        ...
        logger.error(f"[Server](post_log-configuration): { e }")
        return JSONResponse({ "message": "Unsuccess!" }, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)

@app.post("/token", response_model=Token, tags=["Authentication"])
async def login_for_access_token(request : Request, form_data: OAuth2PasswordRequestForm = Depends()):
    try:
        logger.debug(f"[Server](post_token): Accessed from [{ request.client.host }:{ request.client.port }]")
        ...
        logger.error(f"[Server](post_token): { e }")
